...
function do_moodle_stuff($user,$pass){
    global $sitio;
    global $servicio_web;
    $url_moodle = $sitio . '/login/token.php?username=' . $user . '&password='. $pass . '&service=' . $servicio_web;
    //curlear
    $curl = curl_init();
    ...
    if ($obj_moodle_token->{'token'}){
        //Hacer algo lindo
        $token_pu = $obj_moodle_token->{'token'};
        give_me_info($token_pu);
    } else {
...
function give_me_info ($tok){
    global $header_bit;
    global $sitio;
    $url_moodle_rest = $sitio . '/webservice/rest/server.php?wstoken=';
    ...
    $serverurl = $sitio . '/webservice/rest/server.php'. '?wstoken=' . $tok . '&wsfunction='.$functionname;
    $serverurl .= '&forumid=1';
    $curleao = curl_init();
    ...
    for ($n = 0; $n < 5 ; $n++){
        ...
        $link_discussion_f = $sitio . '/mod/forum/discuss.php?d=' . $xml_foro->SINGLE->KEY[0]->MULTIPLE->SINGLE[$n]->KEY[7]->VALUE;
